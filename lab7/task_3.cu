...
int main(void) {
    ...
    // Hint 2: cudaMemcpy
    for(int i = 0; i < 16; i++) {
        cudaMemcpy(device_array_c, device_array_a + i * BUF_2M, BUF_2M * sizeof(int), cudaMemcpyDeviceToDevice);
        cudaMemcpy(device_array_a + i * BUF_2M, device_array_b + i * BUF_2M, BUF_2M * sizeof(int), cudaMemcpyDeviceToDevice);
        cudaMemcpy(device_array_b + i * BUF_2M, device_array_c, BUF_2M * sizeof(int), cudaMemcpyDeviceToDevice);
    }
