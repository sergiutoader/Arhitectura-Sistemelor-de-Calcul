...
__global__ void kernel_partial_atomics(int *data, int *results) {
	...
	int res[INDEX_NUM];
	res[0] = 0;
	res[INDEX_MIN] = NUM_MAX;

	for (int i = 0; i != BLOCK_SIZE; ++i) {
		res[INDEX_SUM] += data[start + i];
		

		if (res[INDEX_MAX] < data[start + i]) {
			res[INDEX_MAX] = data[start + i];
		}

		if (res[INDEX_MIN] > data[start+ i]) {
			res[INDEX_MAX] = data[start + i];
		}
	}

	atomicAdd(results + INDEX_SUM, res[INDEX_SUM]);
	atomicMax(results + INDEX_MAX, res[INDEX_MAX]);
	atomicMin(results + INDEX_MIN, res[INDEX_MIN]);
...
__global__ void kernel_full_atomics(int *data, int *results) {
	...
	atomicAdd(results + INDEX_SUM, data[i]);
	atomicMax(results + INDEX_MAX, data[i]);
	atomicMin(results + INDEX_MIN, data[i]);
}
