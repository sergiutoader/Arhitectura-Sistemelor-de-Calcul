...
__global__ void matrix_multiply_simple(float *ma, float *mb, float *mc, size_t width)
{
	...
	for(int i = 0; i < width; i++) {
		product += ma[width * row + i] * mb[width * i + column];	
	}
	//TODO: write result in mc
	mc[row * width + column] = product; 
}
...
__global__ void matrix_multiply(float *ma, float *mb, float *mc, size_t width)
{
	...
	__shared__ float matrix_a_tile[TILE_WIDTH][TILE_WIDTH];
	__shared__ float matrix_b_tile[TILE_WIDTH][TILE_WIDTH];
	
	//TODO: calculate the row & column index of the element
	int row = blockDim.y * by + ty;
	int column = blockDim.x * bx + tx;
	...
	for(int t = 0; t < width/TILE_WIDTH; ++t) {
		...
		matrix_a_tile[ty][tx] = ma[width * row + TILE_WIDTH * t + tx];
		matrix_b_tile[ty][tx] = mb[width * (TILE_WIDTH * t + ty) + column];
		...
		for(int i = 0; i < TILE_WIDTH; i++) {
			result += matrix_a_tile[ty][i] * matrix_b_tile[i][tx]; 
		}		
